...
void TabFilament::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    std::string resolved_opt_key = opt_key;
    if (m_active_page != nullptr) {
        const auto it = m_active_page->m_opt_id_map.find(opt_key);
        if (it != m_active_page->m_opt_id_map.end())
            resolved_opt_key = it->second;
    }

    const int pos = resolved_opt_key.find("#");

    if (pos > 0) {
        std::string temp_str = resolved_opt_key;
        ...
        std::string opt_key_pure = resolved_opt_key;
