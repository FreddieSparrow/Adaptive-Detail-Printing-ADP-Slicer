...
void Plater::calib_input_shaping_freq(const Calib_Params& params)
{
    ...
    const double outer_wall_speed = print_config->option<ConfigOptionFloat>("outer_wall_speed")->value;
    const double default_acceleration = print_config->option<ConfigOptionFloat>("default_acceleration")->value;
    const double outer_wall_acceleration = print_config->option<ConfigOptionFloat>("outer_wall_acceleration")->value;
    print_config->set_key_value("outer_wall_speed", new ConfigOptionFloat(std::max(outer_wall_speed, 200.0)));
    print_config->set_key_value("default_acceleration", new ConfigOptionFloat(std::max(default_acceleration, 20000.0)));
    print_config->set_key_value("outer_wall_acceleration", new ConfigOptionFloat(std::max(outer_wall_acceleration, 20000.0)));
    print_config->set_key_value("precise_z_height", new ConfigOptionBool(false));
    model().objects[0]->config.set_key_value("brim_type", new ConfigOptionEnum<BrimType>(btOuterOnly));
    model().objects[0]->config.set_key_value("brim_width", new ConfigOptionFloat(3.0));
    model().objects[0]->config.set_key_value("brim_object_gap", new ConfigOptionFloat(0.0));
...
void Plater::Calib_Cornering(const Calib_Params& params)
{
    ...
    const double outer_wall_acceleration = print_config->option<ConfigOptionFloat>("outer_wall_acceleration")->value;
    print_config->set_key_value("outer_wall_speed", new ConfigOptionFloat(std::max(outer_wall_speed, 200.0)));
    print_config->set_key_value("default_acceleration", new ConfigOptionFloat(std::max(default_acceleration, 2000.0)));
    print_config->set_key_value("outer_wall_acceleration", new ConfigOptionFloat(std::max(outer_wall_acceleration, 2000.0)));
    print_config->set_key_value("precise_z_height", new ConfigOptionBool(false));
    model().objects[0]->config.set_key_value("brim_type", new ConfigOptionEnum<BrimType>(btOuterOnly));
    model().objects[0]->config.set_key_value("brim_width", new ConfigOptionFloat(3.0));
    model().objects[0]->config.set_key_value("brim_object_gap", new ConfigOptionFloat(0.0));
